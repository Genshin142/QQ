    ...
    m_lastCheckedPhone("")
{
    ...
    // 初始化 NetworkManager
    NetworkManager::instance().init("127.0.0.1", 8080);
    connect(&NetworkManager::instance(), &NetworkManager::checkPhoneResponse, this, [this](bool exists, const QString& userId) {
        if (exists) {
            downloadAndSetAvatar(userId);
        } else {
    ...
    connect(&NetworkManager::instance(), &NetworkManager::avatarResponse, this, [this](const QString& userId, bool success, const QByteArray& imgData) {
        Q_UNUSED(userId); // load.cpp normally handles current user, but signal provides userId now
        if (success) {
            QPixmap pixmap;
...
void load::checkInputValid()
{
    ...
    if (account.length() == 11) {
        // 11位手机号，仅当账号内容变化时才重新检查（防止密码输入触发）
        if (account != m_lastCheckedPhone) {
            m_lastCheckedPhone = account;
            checkPhoneNumber(account);
        }
    } else {
        // 非11位，清空上次检查记录并显示默认头像
        m_lastCheckedPhone.clear();
        downloadDefaultAvatar();
    }
}
...
void load::checkPhoneNumber(const QString &phoneNumber) {
    writeLoginWindowLog(QString("NetworkManager 检查手机号: %1").arg(phoneNumber), "INFO");
    
    QJsonObject json;
    json["type"] = "check_phone";
...
void load::downloadAndSetAvatar(const QString &user_id) {
    writeLoginWindowLog(QString("NetworkManager 下载头像, 用户ID: %1").arg(user_id), "INFO");
